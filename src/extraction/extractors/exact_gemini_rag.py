# -*- coding: utf-8 -*-
"""
RAG 增强版 Gemini 实体关系抽取脚本
使用 LangChain 检索相关模式作为 few-shot 示例
通过 hiapi.online 的 OpenAI 兼容端点调用 Gemini
"""
import os
import json
import time
...
# OpenAI SDK (用于 Gemini OpenAI 兼容端点)
from openai import OpenAI

# LangChain RAG 组件
# 优先使用新包，若不可用则回退到社区版（使用 importlib 避免静态导入报错）
import importlib
try:
    _hug_mod = importlib.import_module("langchain_huggingface")
    HFEmbeddings = getattr(_hug_mod, "HuggingFaceEmbeddings")
    _HF_IMPL = "langchain_huggingface"
except Exception:
    from langchain_community.embeddings import HuggingFaceEmbeddings as HFEmbeddings
    _HF_IMPL = "langchain_community"
...
# 实验二 Prompt 新路径（仅在 exp02 下使用）
EXP02_PROMPT_FILE = PROJECT_ROOT / "configs" / "prompts" / "exp02" / "prompt.txt"
# 实验三逐论文 Prompt 目录
...
# 输出目录（按实验分流）
OUTPUT_DIR = Path(f"E:/langchain/outputs/extractions/{EXP_ID}/gemini_rag")
LOG_DIR = Path(f"E:/langchain/outputs/logs/{EXP_ID}/gemini")
PROMPT_EXAMPLE_DIR = PROJECT_ROOT / "data" / "prompts" / "gemini_rag_examples"
...
# Gemini 配置
MODEL_NAME = "gemini-2.5-pro"  # 或 gemini-2.0-flash-exp, gemini-1.5-pro
PROVIDER_NAME = "gemini_rag"

print("=" * 80)
print("🚀 RAG 增强版 Gemini 实体关系抽取")
print("=" * 80)
print(f"\n配置:")
...
if RAG_ENABLED:
    try:
        # 校验本地模型路径
        print(f"⏳ 从本地加载 Embeddings ({_HF_IMPL}) ...")
...
# ------------------------------
# 初始化 Gemini 客户端 (通过 hiapi.online)
# ------------------------------
...
client = OpenAI(
    api_key=api_key, 
    base_url="https://hiapi.online/v1"  # hiapi.online OpenAI 兼容端点
)
